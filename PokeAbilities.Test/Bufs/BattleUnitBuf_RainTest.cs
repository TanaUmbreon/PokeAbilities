...
{
    [TestFixture]
    public class BattleUnitBuf_RainTest
    {
        ...
        {
            var buf = new BattleUnitBuf_Rain();
        ...
        {
            var buf = new BattleUnitBuf_Rain() { stack = 0 };
        ...
        {
            var buf = new BattleUnitBuf_Rain() { stack = 6 };
        ...
        {
            var buf = new BattleUnitBuf_Rain() { stack = 5 };
            BattleEmulator.AddBuf(owner, buf);
            Assert.That(buf.stack, Is.EqualTo(5));
        ...
        {
            var buf = new BattleUnitBuf_Rain() { stack = 3 };
            BattleEmulator.AddBuf(owner, buf);
        ...
        [Test(Description = "幕の開始時、手札が0枚の場合はみずタイプが付与されない。")]
        ...
        [Test(Description = "幕の開始時、タイプ付与されていない手札が1枚の場合はその手札にみずタイプ付与される。")]
        ...
        [Test(Description = "幕の開始時、タイプ付与されていない手札が2枚の場合はその手札全てにみずタイプが付与される。")]
        public void TestOnRoundStartAfter_NoTypeHand2()
        {
            ...
            Assert.That(BattleInfo.GetTypeHandCount(owner, PokeType.Water), Is.EqualTo(2));
            Assert.That(BattleInfo.GetMultiTypesHandCount(owner), Is.EqualTo(0));
        }

        [Test(Description = "幕の開始時、タイプ付与されていない手札が3枚の場合は手札2枚にみずタイプが付与される。")]
        ...
        [Test(Description = "幕の開始時、タイプ付与されていない手札が3枚、タイプ付与された手札が1枚の場合は、タイプ付与されていない手札3枚の中から2枚にみずタイプが付与される。")]
        public void TestOnRoundStartAfter_NoTypeHand3AndTypeHand1()
        {
            owner.allyCardDetail.DrawCards(4);
            owner.allyCardDetail.GetHand().FirstOrDefault()?.AddBuf(new BattleDiceCardBuf_Type(PokeType.Grass));
            ...
            Assert.That(BattleInfo.GetTypeHandCount(owner, PokeType.Water), Is.EqualTo(0));
            Assert.That(BattleInfo.GetMultiTypesHandCount(owner), Is.EqualTo(0));
        ...
        [Test(Description = "幕の開始時、タイプ付与された手札が1枚の場合ははみずタイプが付与されない。")]
        ...
        public void TestGetDamageReductionAll_NoTypePageAndAttackDice()
        {
            BattleUnitModel attaker = BattleEmulator.CreateAttacker(
                currentBehaviourDetail: BehaviourDetail.Hit,
                target: owner);
        ...
        {
            BattleUnitModel attaker = BattleEmulator.CreateAttacker(
                currentBehaviourDetail: BehaviourDetail.Evasion,
                target: owner);
        ...
        [Test(Description = "みずタイプ、ほのおタイプ以外のタイプが付与されたバトルページの攻撃ダイスでダメージを受けた時、被ダメージ量は増減しない。")]
        ...
        [Test(Description = "みずタイプ、ほのおタイプ以外のタイプが付与されたバトルページの防御ダイスでダメージを受けた時、被ダメージ量は増減しない。")]
