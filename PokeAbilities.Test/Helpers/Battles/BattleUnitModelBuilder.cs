...
namespace PokeAbilities.Test.Helpers.Battles
{
    ...
    public class BattleUnitModelBuilder
    {
        ...
        public BattleUnitModel Build()
        {
            ...
            //    StageControllerImitator.CreateLibrarianUnit(SephirahType, battleData, Index, Id) :
            //    StageControllerImitator.CreateEnemyUnit(battleData, Index, Id);

            //IEnumerable<BattleDiceCardModelBuilder> fixedDeck = CreateFixedDeck(battleData.unitData.GetDeckSize());
            //BattleAllyCardDetailImitator.Init(unit.allyCardDetail, fixedDeck, unit);
            ...
            {
                StageLibraryFloorModel currentStageFloorModel = StageController.Instance.GetStageModel().GetFloor(BattleEmulator.CurrentFloor);
                var unit = BattleObjectManager.CreateDefaultUnit(Faction);
